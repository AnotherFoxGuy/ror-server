...
{
    auto *dict = new ctemplate::TemplateDictionary("website");
    ...
    ctemplate::TemplateDictionary::SetGlobalValue("TITLE", "Rigs of Rods Server - " + title);
    ctemplate::TemplateDictionary::SetGlobalValue("API_AUTH", Config::getAPIKey());
...
static void show_index(struct mg_connection *conn, const struct mg_request_info *request_info, void *data)
{
    ...
    dict->SetValue("PASSWORD_PROTECTED", Config::getPublicPassword().empty() ? "No" : "Yes");
    dict->SetValue("IP_ADDRESS", Config::getIPAddr() == "0.0.0.0" ? "0.0.0.0 (Any)" : Config::getIPAddr());
    ...
    dict->SetValue("SCRIPTING_ENABLED", Config::getEnableScripting() ? "YES" : "NO");
    ...
    renderTemplate(dict, conn, Config::getResourceDir() + "webserver/templates/index.html");
}

static void show_playerlist(struct mg_connection *conn, const struct mg_request_info *request_info, void *data)
{
    ...
    {
        ctemplate::TemplateDictionary *sub_dict = dict->AddSectionDictionary("PLAYERS");
    ...
    renderTemplate(dict, conn, Config::getResourceDir() + "webserver/templates/playerlist.html");
}
...
static void data_players(struct mg_connection *conn, const struct mg_request_info *request_info, void *data)
{
    ...
    for (auto it = clients.begin(); it != clients.end(); it++)
    {
        ...
        else if (it->GetStatus() == Client::STATUS_USED)
        {
            ...
            for (auto tit = it->streams_traffic.begin(); tit != it->streams_traffic.end(); tit++)
            {
...
static void data_streams(struct mg_connection *conn, const struct mg_request_info *request_info, void *data)
{
    Json::Value rows;
...
static void poweroff(struct mg_connection *conn, const struct mg_request_info *request_info, void *user_data)
{
    char apikey[20];
    if (request_info->query_string != nullptr && sscanf(request_info->query_string, "key=%s", apikey) == 1 &&
