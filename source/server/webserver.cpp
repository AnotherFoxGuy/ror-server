...
int getPlayerColour(int num, char *res) {
  ...
  sprintf(res, "rgb(%d,%d,%d)", (int)(s_colors[num].r * 255.0f),
          (int)(s_colors[num].g * 255.0f), (int)(s_colors[num].b * 255.0f));
  return 0;
}
...
static void data_stats_traffic(struct mg_connection *conn,
...
static void debug_client_list(struct mg_connection *conn,
                              const struct mg_request_info *request_info,
                              void *data) {}
...
static void data_players(struct mg_connection *conn,
  ...
  for (auto it = clients.begin(); it != clients.end(); it++, row_counter++) {
    ...
    if (it->GetStatus() == Client::STATUS_FREE) {
      ...
      for (std::map<unsigned int, stream_traffic_t>::iterator tit =
               it->streams_traffic.begin();
      ...
      for (std::map<unsigned int, RoRnet::StreamRegister>::iterator sit =
               it->streams.begin();
...
static void handle_static_file(struct mg_connection *conn,
                               const struct mg_request_info *request_info,
                               void *data) {
  ...
  if (strcmp(request_info->uri, "/") == 0) {
    ...
    mg_printf(conn, "%s", "HTTP/1.1 200 OK\r\n");
    mg_printf(conn, "%s", "Content-Type: text/html\r\n\r\n");
    mg_write(conn, index_file.begin(), index_file.size());
  } else if (efs.exists(request_info->uri)) {
    auto req_file = efs.open(request_info->uri);
    mg_printf(conn, "%s", "HTTP/1.1 200 OK\r\n");
    if (ends_with(request_info->uri, "html"))
      mg_printf(conn, "%s", "Content-Type: text/html\r\n\r\n");
    else if (ends_with(request_info->uri, "css"))
      mg_printf(conn, "%s", "Content-Type: text/css\r\n\r\n");
    else if (ends_with(request_info->uri, "js"))
      mg_printf(conn, "%s", "Content-Type: text/javascript \r\n\r\n");
    else if (ends_with(request_info->uri, "png"))
      mg_printf(conn, "%s", "Content-Type: image/png\r\n\r\n");
    else if (ends_with(request_info->uri, "jpg"))
      mg_printf(conn, "%s", "Content-Type: image/jpeg\r\n\r\n");
    else
      mg_printf(conn, "%s", "\r\n");
    mg_write(conn, req_file.begin(), req_file.size());
  } else {
...
int StartWebserver(int port, Sequencer *sequencer, bool is_advertised,
  ...
  mg_set_uri_callback(ctx, "/API/data/players/", &data_players, NULL);
  mg_set_uri_callback(ctx, "/API/data/stats/traffic", &data_stats_traffic,
                      NULL);
  mg_set_uri_callback(ctx, "/API/data/configuration/", &data_configuration,
                      NULL);
  mg_set_uri_callback(ctx, "/debug/", &debug_client_list, NULL);
  mg_set_uri_callback(ctx, "*", &handle_static_file, NULL);
